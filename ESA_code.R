...
#the code of this R file finds the matches and mismatches between pairs of datasets (excel spreadsheets) for different variables. 
...
action_type_across_pairs <- action_type_across_pairs_proportions <- diff_action_type_across_pairs <- focus_type_across_pairs <- diff_focus_type_across_pairs <- action_subaction_type_across_pairs <- diff_action_subaction_type_across_pairs <- focus_subfocus_type_across_pairs <- diff_focus_subfocus_type_across_pairs <- vector("list") 
...
for (i in 1:length(data_proportions)) { #every loop corresponds to a pair
  a <- c(a, data_proportions[[i]][["Names"]]) #to get a vector with the names of all of the elements that were used by the five pairs, all together. Some of the names will be repeated because different pairs used the same elements.
  data <- as.data.frame(unique(a)) #to filter out the elements that are repeated and then create a dataframe containing the names of the elements that were used by the five pairs.
